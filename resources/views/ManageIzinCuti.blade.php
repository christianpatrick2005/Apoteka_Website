                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    <div class="flex items-center">
                                        @if($item->kategori == 'izin')
                                            ...
                                            {{ $item->jam_mulai ? \Carbon\Carbon::parse($item->jam_mulai)->format('H:i') : '-' }} <span class="mx-1 text-gray-400">-</span> 
                                            {{ $item->jam_selesai ? \Carbon\Carbon::parse($item->jam_selesai)->format('H:i') : '-' }} WIB
                                        @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex justify-end items-center space-x-2">
                                    ...
                                    @if(auth()->check() && auth()->id() === $item->user_id)
                                        ...
                                        <form action="{{ route('pengajuan-izin.destroy', $item->id) }}" method="POST" class="inline m-0" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengajuan ini?');">
                                            ...
                                            @method('DELETE')
                                            <button type="submit" 
                                                class="inline-flex items-center justify-center p-1.5 rounded-md transition-colors shadow-sm {{ $item->status_pengajuan === 'disetujui' ? 'text-gray-400 bg-gray-100 cursor-not-allowed' : 'text-red-600 bg-red-50 hover:bg-red-100' }}" 
                                                title="{{ $item->status_pengajuan === 'disetujui' ? 'Tidak dapat dihapus karena sudah disetujui' : 'Hapus' }}"
                                                {{ $item->status_pengajuan === 'disetujui' ? 'disabled' : '' }}>
